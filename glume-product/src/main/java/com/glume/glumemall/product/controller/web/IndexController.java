...
public class IndexController {
    ...
    public String park() throws InterruptedException {
        ...
        park.acquire(5);     //获取一个信号、获取一个值,占一个车位
        boolean flag = park.tryAcquire();
        if (flag) {
            //执行业务
            System.out.println("车进入场内，车位减一");
        } else {
            return "没有车位";
        }
        return "ok=>" + flag;
